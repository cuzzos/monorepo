# iOS App - Automatic Workflow

This iOS app uses **XcodeGen + UniFFI** for automatic Rust-Swift integration.

## 🚀 Quick Setup

### 1. Prerequisites

```bash
# Install XcodeGen (one-time)
brew install xcodegen
```

### 2. Initial Build

Build Rust libraries (one-time):

```bash
cd ../shared
./build-ios.sh
```

### 3. Generate Xcode Project

```bash
cd ../ios
xcodegen generate
```

This creates `Thiccc.xcodeproj` with:
- ✅ Automatic Rust rebuilds
- ✅ Automatic UniFFI binding generation
- ✅ Pre-build scripts configured

### 4. Open & Run

```bash
open thiccc/Thiccc.xcodeproj
```

**That's it!** Hit ⌘R and Xcode automatically:
- Checks if Rust code changed
- Rebuilds Rust libraries if needed
- Regenerates Swift bindings
- Builds and runs the app

---

## 📁 Structure

```
ios/
├── README.md                       ← You are here
├── project.yml                     ← XcodeGen specification
│
├── Thiccc.xcodeproj/               ← Generated by xcodegen
│   └── project.pbxproj             ← (regenerated as needed)
│
└── thiccc/                         ← Source files
    ├── Thiccc/
    │   ├── ThicccApp.swift         ✅ App entry point
    │   ├── CoreUniffi.swift        ✅ Rust bridge
    │   ├── Views/                  ✅ SwiftUI views
    │   ├── Assets.xcassets/        ✅ Assets
    │   └── Generated/              ✅ UniFFI bindings (auto-generated)
    └── Thiccc.xcodeproj/
```

---

## 🔧 Daily Workflow

### Make Rust Changes

```bash
cd ../shared
vim src/lib.rs  # Edit Rust code
```

### Build in Xcode

```bash
open thiccc/Thiccc.xcodeproj
# Hit ⌘R - everything rebuilds automatically!
```

**No manual steps needed!** Xcode's pre-build script handles:
1. Checking if Rust changed
2. Rebuilding Rust library
3. Regenerating Swift bindings
4. Compiling and running

---

## ⚙️ Configuration

### Signing

First time setup:
1. Open project in Xcode
2. Select **Thiccc** target
3. Go to **Signing & Capabilities**
4. Enable "Automatically manage signing"
5. Select your Apple Developer team

### Modify Project Structure

Edit `project.yml`, then:

```bash
xcodegen generate
# Xcode will prompt to reload - click "Reload"
```

---

## 🆘 Troubleshooting

### "XcodeGen not found"
```bash
brew install xcodegen
```

### "Homebrew not found"
```bash
/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"
```

### "No such module 'SharedCore'"
Xcode's pre-build script will generate it on first build. Just hit ⌘B (Build).

### Pre-build script fails
Make sure Rust toolchain is installed:
```bash
rustup show
rustup target add aarch64-apple-ios-sim
```

### Files appear red in Xcode
Regenerate the project:
```bash
xcodegen generate
```

---

## 📚 Learn More

- **Main README**: `../../README.md` - Complete project overview
- **UniFFI Migration**: `../../UNIFFI-MIGRATION.md` - Detailed guide
- **XcodeGen**: https://github.com/yonaskolb/XcodeGen
- **TestFlight**: `../TESTFLIGHT-QUICKSTART.md` - Deployment guide

---

**Ready to deploy?** → See `../TESTFLIGHT-QUICKSTART.md`

